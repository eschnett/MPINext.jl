...
aint_count_kinds = [
    "POLYDISPLACEMENT_AINT_COUNT", "POLYDISPOFFSET", "POLYDTYPE_PACK_SIZE", "POLYDTYPE_STRIDE_BYTES", "POLYLOCATION"
]
...
function ensure_comm!(state, input_conversions)
    state.have_comm[] && return nothing
...
function ensure_comm_size!(state, input_conversions)
    state.have_comm_size[] && return nothing
...
function ensure_comm_rank!(state, input_conversions)
    state.have_comm_rank[] && return nothing
    ensure_comm!(state, input_conversions)
    append!(
        input_conversions,
        [
...
append!(f_interfaces, ["module mpi_functions", "  implicit none", "  public", "  save", "", "  interface"])

append!(f08_implementations_useonly, ["module mpi_f08_functions", "  use mpi, only: &"])
append!(f08_implementations_public, ["    MPI_VERSION", "  implicit none", "  private", "  save", ""])
append!(f08_implementations_body, ["", "contains"])
        ...
        for parameter in parameters
                ...
                if param_direction == "in"
                    if "f90_parameter" ∉ suppress
                        ...
                        if root_only
                            ...
                            append!(output_conversions, ["if (q_comm_rank == 0)", "  free(c_$parname);"])
                        else
            ...
            elseif kind ∈ ["STRING_ARRAY"]
                ...
                if "f90_parameter" ∉ suppress
                    ...
                    append!(output_conversions, ["for (size_t n=0; n<count_$parname; ++n)", "  free(argv_$parname[n]);"])
                else
                    push!(call_arguments, "NULL")
                end
                if "f90_parameter" ∉ suppress
...
append!(f_interfaces, ["", "  end interface", "", "end module mpi_functions"])

append!(f08_implementations_body, ["", "end module mpi_f08_functions"])
